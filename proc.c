...
void
changetickets(int pid, int numticks){
  struct proc *p;
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->pid == pid){
      p->tickets = numticks;
...
void lottery(){
  ...
  srand(time(0));
  int r = rand() % total;
  printf("lottery random number:%d\n", r);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->tickets != 0){
      if(r<= p->tickets+prevnum
       && r>prevnum){
        curr_proc = p;
  ...
  printf("lottery selected pid:%d ticketnum:%d\n", answer->pid, answer->tickets);
}
