...
enum cmds        { FORK=0, SETPID,   SHOWPID,   WAIT,   EXIT,   SLEEP,   WAKEUP,   PS,   SCHEDULE,  TIMER,    HELP,   QUIT,   TICKET, LOTTERY };
char *cmdstr[] = {"fork", "Setpid", "currpid",  "wait", "exit", "sleep", "wakeup", "ps",   "schedule", "timer", "help", "quit", "ticket", "lottery"};
...
int executeCmd(char** params, int nparams)
{
    ...
    case TICKET:
        ...
        scanf("%d", &t);
        //placed to clear input buffer from enter key
        char ch = getchar();
        changetickets(id,t);
        break;
    case LOTTERY:
