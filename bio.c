...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "create") == 0) { // create fs file
        ...
        */
        ...
        int fd5 = tfs_open("marksfile",TO_CREATE | TO_RDWR, 0);
        printf("fd5: %d\n", fd5);
        s = tfs_write(fd5, "Writing data to marks file. datadatadatadatadata", 48);
        printf("tfs_write bytes: %d\n", s);
        ...
        int fd5 = tfs_open("marksfile", TO_RDONLY, 0);
        printf("fd5: %d\n", fd5);
        s = tfs_read(fd5,buffer,47);
         printf("tfs_read bytes: fd: %d, bytes read: %d value read: %s\n", fd5, s, buffer);
